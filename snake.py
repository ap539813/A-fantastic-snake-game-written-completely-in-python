...
from important_variables import screen_width, screen_height, gameWindow, snake_colour, text_colour, score_board_size, apple_width
...
while not exit_game:
    ...
    if (snake_x + snake_width//2) in range(food_pos[0], food_pos[0] + apple_width) and (snake_y + snake_width//2) in range(food_pos[1], food_pos[1] + apple_width):
        snake_len += 1
        sound1.play()
    ...
    if (head[-1][0] >= (screen_width - snake_width)) or (head[-1][0] <= 1) or (head[-1][1] >= screen_height - snake_width) or (head[-1][1] <= score_board_size) or (head[-1] in head[:-1]):
        ...
        while game_over == True:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        ...
                        # gameWindow.fill((255, 255, 255))
                        draw_background(gameWindow)
                        put_text('score : ' + str(snake_len - 1), text_colour, 10, 10, font, gameWindow)
                        put_text('highest score : ' + str(high_score), text_colour, screen_width//2, 10, font, gameWindow)
                        ...
                        plot_snake(gameWindow, head, snake_width, snake_colour, snake_len, snake_mv)
                        pygame.display.update()
                        clock.tick(FPS)
                        game_over = False
                        continue
            ...
            put_text('Game over', text_colour, 200, 540, font2, gameWindow)
            put_text('press ENTER to restart or any other key', text_colour, 20, 600, font, gameWindow)
        # break
    put_text('score : ' + str(snake_len - 1), text_colour, 10, 10, font, gameWindow)
    put_text('highest score : ' + str(high_score), text_colour, screen_width//2, 10, font, gameWindow)
